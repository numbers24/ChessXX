...
public class Main {
    ...
    public static boolean enpassant(int i, int j, int k, String colorturn){
        ...
        switch (colorturn){
            case "##" : if(j==3) break; else return false;
            case "  " : if(j==4) break; else return false;
        }
    ...
    public static void castle(int i, int j, int j2, int k){
        ...
        board[i][j].piece = board[i][j2].piece;
        ...
        {   //check if the player is trying to put a piece on place already occupied by one of his own
            ...
            if(A instanceof Pawn) //reverse en passant
            {
                ...
                try {
                    ...
                    if (j2 == 0 || j2 == 7) {//promotion time
                        switch(promotion){
                            case "Q" : A = new Queen(A.name.charAt(0)+"Q", A.color,A.moveCount); break;
                            case "R" : A = new Rook(A.name.charAt(0)+"R", A.color,A.moveCount); break;
                            case "B" : A = new Bishop(A.name.charAt(0)+"B", A.color,A.moveCount); break;
                            case "K" : A = new Knight(A.name.charAt(0)+"K", A.color,A.moveCount); break;
                        }
                    }
                } catch (Exception e) {A = new Queen(A.name.charAt(0)+"Q", A.color,A.moveCount);}//default promotion queen
            }
